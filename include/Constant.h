...
constexpr static class GetConstant_t {
	template <class V> static TypedConstant<decltype(V::value), V::value> sfinae(int);
	template <class V> static TypedConstant<const decltype(V::value)&, V::value> sfinae(long);
public:
	template <class V> constexpr auto operator()(V v) const { return decltype(sfinae<V>(0)){}; }
} GetConstant{};
